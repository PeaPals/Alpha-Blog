...
RSpec.describe User, type: :model do
  context 'username validation tests' do
    ...
    it 'ensure unique value' do
      user.username = 'alphablog'
      expect(user.save).to be_truthy
      user2 = User.new(username: 'alphablog', email: 'alphablog@test.com', password: 'abcdef')
  ...
  context 'validation tests complete' do
    let(:user) {User.new(username: 'alphablog', email: 'test@example.com', password: '1234567')}
